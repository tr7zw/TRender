 ...
 *
 * <p>
 * Item slot widgets can contain multiple visual slots themselves. For example,
 * a slot widget might be 5x3 with 15 visual slots in total.
 *
 * <p>
 * Item slots are handled with so-called peers in the background. They are
 * instances of {@link ValidatedSlot} that handle the interactions between the
 * player and the widget.
 *
 * <h2>Filters</h2> Item slots can have filters that check whether a player is
 * allowed to insert or take out an item or not. The filters can be set with
 * {@link #setInputFilter(Predicate)} and {@link #setOutputFilter(Predicate)}.
 * For example:
 *
 * <pre>
 * {@code
 ...
 * </pre>
 *
 * <h2>Listeners</h2> Slot change listeners are instances of
 * {@link WItemSlot.ChangeListener} that can handle changes to item stacks in
 * slots. For example:
 *
 * <pre>
 * {@code
...
public class WItemSlot extends WWidget {
    ...
    /**
     * {@return whether this slot is a big slot} Big slots are commonly used for
     * crafting results and similar outputs.
     */
    ...
    /**
     * Sets the icon to this slot. Can be used for labeling slots for certain
     * activities.
     *
     * @param icon the icon
     ...
     *
     * @return {@code true} if the icon should always be painted, {@code false}
     *         otherwise
     * @since 9.1.0
     */
     ...
     * contains an item.
     *
     * @param iconOnlyPaintedForEmptySlots {@code true} if the icon should always be
     *                                     painted, {@code false} otherwise
    ...
    /**
     * Returns true if the contents of this {@code WItemSlot} can be modified by
     * players.
     *
     * @return true if items can be inserted into or taken from this slot widget,
     *         false otherwise
     * @since 1.8.0
     */
    ...
    public void validate(GuiDescription host) {
        ...
        for (int y = 0; y < slotsHigh; y++) {
            for (int x = 0; x < slotsWide; x++) {
                ...
                ValidatedSlot slot = createSlotPeer(inventory, index, this.getAbsoluteX() + (x * 18) + 1,
    ...
    /**
     * Gets the starting {@linkplain net.minecraft.world.inventory.Slot#index ID}
     * for the slot peers.
     *
     * @return the starting ID for the slot peers, or -1 if this slot widget has no
     *         peers
     * @since 11.1.0
     */
    ...
    /**
     * Adds a change listener to this slot. Does nothing if the listener is already
     * registered.
     *
     ...
     * index. One example of a narration name would be "hotbar" for the player's
     * hotbar.
     *
