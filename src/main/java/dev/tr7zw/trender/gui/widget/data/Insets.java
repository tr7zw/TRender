...
/**
 * The layout insets of a panel. The insets describe how many pixels should be
 * around the panel's contents. For example, root panels have 7 pixels around
 * their contents, which is set via {@link #ROOT_PANEL}.
 *
...
public record Insets(int top, int left, int bottom, int right) {
    ...
    /**
     * The default insets of a root panel, providing 7 pixels around the content on
     * all sides.
     */
    ...
    public Insets {
        if (top < 0)
            throw new IllegalArgumentException("top cannot be negative, found " + top);
        if (left < 0)
            throw new IllegalArgumentException("left cannot be negative, found " + left);
        if (bottom < 0)
            throw new IllegalArgumentException("bottom cannot be negative, found " + bottom);
        if (right < 0)
            throw new IllegalArgumentException("right cannot be negative, found " + right);
    }
     ...
     *
     * <p>
     * Equivalent to <code>{@link #left()} + {@link #right()}</code>.
     *
     * @since 9.1.0
     */
    public int width() {
     ...
     *
     * <p>
     * Equivalent to <code>{@link #top()} + {@link #bottom()}</code>.
     *
     * @since 9.1.0
     */
    public int height() {
