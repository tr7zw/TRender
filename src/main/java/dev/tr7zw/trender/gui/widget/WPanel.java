...
public abstract class WPanel extends WWidget {
    ...
    /**
     * Uses this Panel's layout rules to reposition and resize components to fit
     * nicely in the panel.
     */
    public void layout() {
     ...
     *
     * <p>
     * Subclasses should call {@code super.validate(c)} to ensure that children are
     * validated.
     *
     ...
     *
     * <p>
     * Subclasses should call {@code super.addPainters()} to ensure that children
     * have proper default painters.
     *
     * @since 3.0.0
     */
    ...
    public String toString() {
        return getClass().getSimpleName() + " {\n"
                + children.stream().map(Object::toString).map(x -> x + ",")
                        .flatMap(x -> Stream.of(x.split("\n")).filter(y -> !y.isEmpty()).map(y -> "\t" + y))
                ...
                if (widget == parent) {
                    if (n == 0) {
                        ...
                        throw new IllegalArgumentException(
                                "Adding level " + n + " parent recursively to " + owner + ": " + widget);
                    }
                }
